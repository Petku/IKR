...
count_of_gausses_for_one_person = 5
...
score = []
for i, test_person in enumerate(tests_persons):
    ll_m = logpdf_gmm(test_person.values(), persons_gmms[i].weight, persons_gmms[i].mean, persons_gmms[i].cov)
    score.append((sum(ll_m) + np.log(0.03)))
